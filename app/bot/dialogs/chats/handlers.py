...
async def save_chat_from_copy(
    ...
    logger.debug(
        f'Выбран чат, откуда копировать, с id={item_id}'
    )  # TODO: поменять на название из БД
    dialog_manager.dialog_data['chat_from_id'] = (
        item_id  # TODO: поменять на название
    )
...
async def copy_messages(
    ...
    logger.debug(
        f'Выбран чат, куда копировать, с id={item_id}'
    )  # TODO: поменять на название
    bot = dialog_manager.event.bot
    msg = await bot.send_message(
        chat_id=callback.from_user.id, text=labels_texts.COPY_MESSAGES_PROCESS
    )
    if dialog_manager.dialog_data.get('chat_from_id') == item_id:
        await callback.answer(
            text=labels_texts.ALERT_COPY_CHAT, show_alert=True
        )
        return
    ...
    dialog_manager.dialog_data['chat_in_id'] = (
        item_id  # TODO: поменять на название
    )
...
async def set_chat_for_del(
    ...
    dialog_manager.dialog_data['chat_del_name'] = (
        'Название чата'  # TODO: поменять на название из БД
    )
...
async def del_chat(
    ...
    bot = dialog_manager.event.bot
    msg = await bot.send_message(
        chat_id=callback.from_user.id, text=labels_texts.DEL_CHAT_PROCESS
    )
    logger.info(
...
async def find_chat(
    ...
    bot = dialog_manager.event.bot
    msg = await bot.send_message(
        chat_id=message.from_user.id, text=labels_texts.FIND_CHAT_PROCESS
    )
    # TODO: поиск чатов в БД
    found_chats = [
        'Базовый Python 2026 1 поток',
        'Базовый Python 2026 2 поток',
