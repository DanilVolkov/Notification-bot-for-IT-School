...
async def get_del_chat_name(dialog_manager: DialogManager, **kwargs):  # TODO: добавить именно название чата, а не его id
    ...
    if dialog_manager.dialog_data.get('chat_del_confirm'):
        dialog_manager.dialog_data.clear()
    return {
        'chat_del_id': chat_del_id,
        'chat_del_name': chat_del_name
    }


async def get_found_chats(dialog_manager: DialogManager, **kwargs):
    found_chats = dialog_manager.dialog_data.get('found_chats')
    found_chats = [(chat_name, index) for index, chat_name in enumerate(found_chats)]
    dialog_manager.dialog_data.clear()
