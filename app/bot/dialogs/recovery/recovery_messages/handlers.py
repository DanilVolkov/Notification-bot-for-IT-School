...
async def set_chat_messages_for_recovery(callback: CallbackQuery, widget: Select, dialog_manager: DialogManager, item_id: str):
    chat_recovery_msgs_id = item_id  # TODO: поменять на id из БД
    chat_recovery_msgs_name = 'Python для начинающих'  # TODO: поменять на название из БД
    dialog_manager.dialog_data['chat_recovery_msgs_id'] = chat_recovery_msgs_id
    dialog_manager.dialog_data['chat_recovery_msgs_name'] = chat_recovery_msgs_name
...
async def set_del_message_info(callback: CallbackQuery, widget: Select, dialog_manager: DialogManager, item_id: str):
...
async def recovery_message(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    pass