...
async def del_message(
    ...
    msg = await bot.send_message(
        chat_id=callback.from_user.id, text=labels_texts.DEL_MESSAGE_PROCESS
    )
...
async def update_message_datetime(
    message: Message,
    widget: ManagedTextInput,
    dialog_manager: DialogManager,
    text: str,
...
async def update_message_without_datetime(
    callback: CallbackQuery, button: Button, dialog_manager: DialogManager
):
    # TODO: изменение даты в БД
