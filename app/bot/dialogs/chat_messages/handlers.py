...
async def start_message_info_dialog(
    ...
    await dialog_manager.start(
        MessageInfoSG.start, data={'message_id': item_id, 'chat_id': chat_id}
    )


async def save_message_name(
    message: Message,
...
async def save_message_datetime_and_start_message_info_dialog(
    message: Message,
    widget: ManagedTextInput,
    dialog_manager: DialogManager,
    text: str,
    ...
    await dialog_manager.start(
        MessageInfoSG.start, data={'message_id': 5, 'chat_id': chat_id}
    )


async def save_message_without_datetime_and_start_message_info_dialog(
...
async def find_message(
    ...
    msg = await bot.send_message(
        chat_id=message.from_user.id, text=labels_texts.FIND_MESSAGE_PROCESS
    )
...
async def download_msgs_from_excel(
    ...
    if document.file_name.split('.')[-1].endswith('xlsx'):
        msg = await bot.send_message(
            ...
            text=labels_texts.DOWNLOAD_MSGS_FROM_EXCEL_PROCESS,
        )
        ...
        if not is_valid:
            ...
            await dialog_manager.switch_to(
                ChatMessagesSG.download_msgs_from_excel_done
            )
        ...
        try:
            ...
            logger.warning(
                f'Недостаточно прав для удаления файла: {file_path}'
            )
