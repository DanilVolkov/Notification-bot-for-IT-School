...
def get_sticker_from_status_message(status: MessageStatus):
    match status:
        case MessageStatus.SENT: return labels_texts.MESSAGE_STICKER_SENT
        case MessageStatus.PLANNED: return labels_texts.MESSAGE_STICKER_PLANNED
        case MessageStatus.DRAFT: return labels_texts.MESSAGE_STICKER_NONE
        case _: return ''


def get_text_from_status_message(status: MessageStatus):
    match status:
        case MessageStatus.SENT: return labels_texts.MESSAGE_TEXT_SENT
        case MessageStatus.PLANNED: return labels_texts.MESSAGE_TEXT_PLANNED
        case MessageStatus.DRAFT: return labels_texts.MESSAGE_TEXT_NONE
        case _: return ''
...
def parsing_message_status(status: str):
    status = status.strip()
    match status:
        case "Отправлено": return MessageStatus.SENT
        case "Запланировано": return MessageStatus.PLANNED
        case _: return MessageStatus.DRAFT
...
async def get_found_messages(dialog_manager: DialogManager, **kwargs):
    ...
    return {"found_messages": found_messages, "is_found_messages": is_found_messages, "is_not_found_messages": not is_found_messages}
